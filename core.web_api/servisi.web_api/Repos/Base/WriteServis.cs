...
public class WriteServis<T, Tdb, TUpsert> : IWriteServis<T, Tdb, TUpsert> where T : class where Tdb : class where TUpsert : class
{
    ...
    public WriteServis(IMapper _mapper, DatabaseContext context, IConfiguration _config, ILogger _logger)
    {
        Context = context;
        config = _config;
        logger = _logger;
